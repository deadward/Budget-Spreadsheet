...
# comment
def create_calendar(sheet, calendar_start_row):
    # Calendar Headers
    sheet[f"A{calendar_start_row}"] = "Date"
    sheet[f"B{calendar_start_row}"] = "Transaction Name"
    sheet[f"C{calendar_start_row}"] = "Amount"
    sheet[f"D{calendar_start_row}"] = "Checking Total"
    sheet[f"E{calendar_start_row}"] = "Additional Amount"
    sheet[f"F{calendar_start_row}"] = "Grand Total"

    # Apply formatting to headers
    for col in range(1, 7):
        cell = sheet[f"{get_column_letter(col)}{calendar_start_row}"]
        cell.font = Font(bold=True)
        cell.alignment = Alignment(horizontal="center", vertical="center")

    # Add "Pending" row under the headers
    pending_row = calendar_start_row + 1
    sheet[f"A{pending_row}"] = "Pending"
    sheet[f"A{pending_row}"].alignment = Alignment(horizontal="left")
    sheet[f"D{pending_row}"] = f"=B3+C23"
    sheet[f"D{pending_row}"].number_format = numbers.FORMAT_CURRENCY_USD_SIMPLE
    sheet[f"F{pending_row}"] = f"=D23+B4+B5+B6+B7"
    sheet[f"F{pending_row}"].number_format = numbers.FORMAT_CURRENCY_USD_SIMPLE
    ...
    while current_date <= end_date:
        ...
        if current_date.day == 1:
            sheet[f"A{current_row}"] = current_date.strftime("%B %d")  # Month and day for the 1st of each month
        else:
            sheet[f"A{current_row}"] = current_date.day  # Day of the month
        ...
        if start_adding_pay is True:
            if payday == 0:
                ...
                sheet[f"B{current_row}"] = "Pay"
                sheet[f"C{current_row}"] = f"=E3"
                sheet[f"C{current_row}"].number_format = numbers.FORMAT_CURRENCY_USD_SIMPLE
                sheet[f"E{current_row}"] = f"=E4+E5+E6"
                sheet[f"E{current_row}"].number_format = numbers.FORMAT_CURRENCY_USD_SIMPLE

                # Add formulas and formatting for Amount and Additional Amount
                apply_formulas(sheet, current_row)
        ...
        if current_date in frontier_dates:
            ...
            sheet[f"B{current_row}"] = "Frontier Waste"
            sheet[f"C{current_row}"] = f"=B20"
            sheet[f"C{current_row}"].number_format = numbers.FORMAT_CURRENCY_USD_SIMPLE
        ...
        bills_to_add = check_bill_date(current_date.day)
        ...
        if bills_to_add:
            ...
            for bill in bills_to_add:
                ...
                sheet[f"B{current_row}"] = bill

                # Dynamically reference the amount from the Income table
                sheet[f"C{current_row}"] = f"=B{bill_column(bill)}*-1"
                sheet[f"C{current_row}"].number_format = numbers.FORMAT_CURRENCY_USD_SIMPLE
    ...
    apply_conditional_formatting1(sheet, col="D", start_row=23, end_row=current_row-1)
    apply_conditional_formatting2(sheet, col="F", start_row=23, end_row=current_row-1)
...
def apply_formulas(sheet, current_row):
    # Add formulas and formatting for Amount and Additional Amount
    sheet[f"D{current_row}"] = f"=D{current_row - 1}+C{current_row}"
    sheet[f"D{current_row}"].number_format = numbers.FORMAT_CURRENCY_USD_SIMPLE
    sheet[f"F{current_row}"] = f"=C{current_row}+E{current_row}+F{current_row - 1}"
    sheet[f"F{current_row}"].number_format = numbers.FORMAT_CURRENCY_USD_SIMPLE


def apply_conditional_formatting1(sheet, col, start_row, end_row):
    ...
    sheet.conditional_formatting.add(cell_range, burn_orange_rule)
    sheet.conditional_formatting.add(cell_range, orange_rule)
    sheet.conditional_formatting.add(cell_range, yellow_rule)
    sheet.conditional_formatting.add(cell_range, light_green_rule)
    sheet.conditional_formatting.add(cell_range, green_rule)
    sheet.conditional_formatting.add(cell_range, neon_green_rule)

def apply_conditional_formatting2(sheet, col, start_row, end_row):
    # Define the column range (e.g., B2:B10)
    cell_range = f"{col}{start_row}:{col}{end_row}"
